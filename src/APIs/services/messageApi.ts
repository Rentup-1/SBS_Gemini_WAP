...
const mapApiMessageToMessage = (apiMessage: ApiMessage): Message => {
  ...
  const source: "WAP" | "Website" = apiMessage.phone.includes("+20")
    ? "WAP"
    : "Website";

  // Map type from API to our local types
  const type: "Inventory" | "Request" =
    apiMessage.type === "inventory" ? "Inventory" : "Request";

  // Determine user type (you might want to adjust this logic)
  const userType: "New" | "Existing" =
    apiMessage?.user?.created_at &&
    apiMessage.user.created_at >
  ...
  const replied: "Yes" | "No" =
    apiMessage.message.includes("replied") ||
    apiMessage.message.includes("contacted")
      ? "Yes"
      : "No";

  // Determine status (you might need additional logic)
  const status: "Not Listed" | "Listed" =
    apiMessage.type === "inventory" ? "Listed" : "Not Listed";

  return {
    id: apiMessage.id,
    contactName:
      apiMessage?.user?.name || apiMessage.username || "Unknown User",
