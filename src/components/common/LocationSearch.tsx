...
export const LocationSearch: React.FC<LocationSearchProps> = (props) => {
  ...
  const fetchLocations = async (query: string) => {
    ...
    try {
      const response = await fetch(
        `https://sbsapi.rentup.com.eg/api/locations/autocomplete?search=${encodeURIComponent(
          query
        )}&limit=10`
      );
  ...
  const toggleLocationMulti = (location: Location) => {
    if (mode === "multi") {
      const isSelected = props.selectedLocations?.some(
        (item) => item.id === location.id
      );

      let newSelected: Location[];
      if (isSelected) {
        newSelected = (props.selectedLocations || []).filter(
          (item) => item.id !== location.id
        );
      } else {
  ...
  const removeLocation = (locationId: string) => {
    if (mode === "multi") {
      props.onChange(
        (props?.selectedLocations || []).filter(
  ...
  const isLocationSelected = (locationId: string) => {
    if (mode === "multi" && props.selectedLocations) {
      return props.selectedLocations.some(
        (item) => String(item.id) === locationId
      );
    }
    return false;
        ...
        >
          {locations.map((location) => (
            <button
              ...
              onClick={() =>
                mode === "single"
                  ? handleSelectSingle(location)
                  : toggleLocationMulti(location)
              }
