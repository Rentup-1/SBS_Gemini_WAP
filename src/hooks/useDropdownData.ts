...
import {
  ...
  fetchPropertyTypes,
  fetchTags,
} from "../APIs/services/propertyApi";
...
const HARDCODED_DATA = {
  ...
  clientNames: [
    "Ahmed Said (01012345678)",
    "Jane Doe (01198765432)",
    "Mohamed Ali (01234567890)",
    "New Client",
  ],
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to fetch dropdown data";
        ...
        setDropdownOptions({
          ...HARDCODED_DATA,
          listedByUsers: HARDCODED_DATA.listedByUsers.map((name, index) => ({
            id: index,
            name,
          })),
        });
